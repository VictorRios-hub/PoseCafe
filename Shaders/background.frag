...
float f(in vec2 p)
{
    return sin(p.x+sin(p.y+u_time*velocity_x)) * sin(p.y*p.x*0.1+u_time*velocity_y);
}
...
Field field(in vec2 p,in int mode)
{
    ...
    if(mode == 0){
        ...
        for( int i=0; i<iterationTime1; i++ )
        {
            ...
            p.x = p.x + sin( u_time*mode_2_speed/10.)/10.;
            p.y = p.y + cos(u_time*mode_2_speed/10.)/10.;
            rz= g; 
        }
        field.vel = rz;
        return field;
    ...
    if(mode == 1){
        ...
        for(int i=1; i<iterationTime2; i++){
            ...
            p.x+=0.3/float(i)*sin(float(i)*3.*p.y+u_time*mode_2_speed) + 0.5;
            p.y+=0.3/float(i)*cos(float(i)*3.*p.x + u_time*mode_2_speed) + 0.5;
        }
        field.pos = p;
        return field;
    }
...
vec3 getRGB(in Field fld,in int mode){
    ...
    if(mode == 0){
        ...
        origCol =(p.x+p.y)*vec3(139., 69., 19.)/139.;
        return origCol;
    }
